...
def run_demo():
	...
	try:
		...
		while calibrated_device_count < len(device_manager._available_devices):
			...
			object_point = pose_estimator.get_chessboard_corners_in3d() #corner point
			calibrated_device_count = 0
		...
		while 1:
			 # Get the frames from all the devices
				...
				bounding_box_points_color_image, mm_box_points_color_image, area_rect, height = calculate_boundingbox_points(point_cloud, calibration_info_devices,mm_roi_2d=roi_2D )
				
				# Draw the bounding box points on the color image and visualise the results
				visualise_measurements(frames_devices, bounding_box_points_color_image, mm_box_points_color_image, area_rect, height)
